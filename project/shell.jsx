...
;(function () {
  ...
  function Sidebar({ route, go }) {
    ...
    { id: 'dashboard', icon: 'layout-dashboard', label: 'Panel de control' },
    { id: 'intake', icon: 'log-in', label: 'Registrar ingreso' },
    { id: 'inspection', icon: 'clipboard-check', label: 'Inspección' },
    { id: 'detail', icon: 'wrench', label: 'Reparaciones', badge: { value: 57 } },
    { id: 'repairs', icon: 'hammer', label: 'Registro de trabajo' }];

    const ops = [
    { id: 'yard', icon: 'container', label: 'Patio' },
    ...
    return (
      ...
      <nav style={shellStyles.nav}>
        <div style={shellStyles.navSection}>TALLER</div>
        {items.map((it) => <NavItem key={it.id} {...it} active={route === it.id} onClick={() => go(it.id)} />)}
        <div style={shellStyles.navSection}>DESPACHO</div>
